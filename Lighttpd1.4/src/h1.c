...
h1_send_1xx_info (request_st * const r, connection * const con)
{
    ...
    int rc = connection_write_cq(con, cq, MAX_WRITE_LIMIT);
...
void h2_upgrade_h2c (request_st * const r, connection * const con);
__attribute_cold__
static int
...
h1_reqbody_read (request_st * const r)
{
    ...
    if (con->is_readable > 0) {
        ...
        switch(network_read_dispatch(con, cq, max_per_read)) {
        case -1:
            request_set_state_error(r, CON_STATE_ERROR);
