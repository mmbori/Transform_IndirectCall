...
// inlining_added
off_t mod_deflate_using_libdeflate_setjmp_cb(void *dst, const void *src, off_t len);
off_t network_write_setjmp_write_cb(void *dst, const void *src, off_t len);

/* 인라인 디스패치 함수 */
static inline off_t
setjmp_eval3_dispatch(off_t(*cb)(void *, const void *, off_t), void *dst, const void *src, off_t len)
{
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    // if (cb == mod_deflate_using_libdeflate_setjmp_cb) {
    //     return mod_deflate_using_libdeflate_setjmp_cb(dst, src, len);
    // }
    // if (cb == network_write_setjmp_write_cb) {
    //     return network_write_setjmp_write_cb(dst, src, len);
    // }
}
...
off_t sys_setjmp_eval3(off_t(*cb)(void *, const void *, off_t), void *dst, const void *src, off_t len)
{
    off_t rv;
    if_SYS_SETJMP_TRY()
        rv = setjmp_eval3_dispatch(cb, dst, src, len);
