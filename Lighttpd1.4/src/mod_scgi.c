...
int scgi_env_add_scgi(void *venv, const char *key, size_t key_len, const char *val, size_t val_len) {
	buffer *env = venv;
	size_t len;
...
int scgi_env_add_uwsgi(void *venv, const char *key, size_t key_len, const char *val, size_t val_len) {
	if (!key || (!val && val_len)) return -1;
...
// inlining_added
static inline int
scgi_env_add_dispatch(void *venv, const char *key, size_t key_len, const char *val, size_t val_len, int proto) {
    /* 프로토콜에 따른 최적화된 디스패치 */
    if (proto == LI_PROTOCOL_SCGI) {
        return scgi_env_add_scgi(venv, key, key_len, val, val_len);
    }
    else if (proto == LI_PROTOCOL_UWSGI) {
        return scgi_env_add_uwsgi(venv, key, key_len, val, val_len);
    }
    
    return -1; /* 지원하지 않는 프로토콜 */
}

handler_t scgi_create_env(handler_ctx *hctx) {
	...
	if (hctx->conf.proto == LI_PROTOCOL_SCGI) {
		...
		size_t len;
		scgi_env_add_dispatch(b, CONST_STR_LEN("SCGI"), CONST_STR_LEN("1"), hctx->conf.proto);
		buffer_clear(tb);
