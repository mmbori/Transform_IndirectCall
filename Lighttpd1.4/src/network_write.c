...
int network_write_chunkqueue_writev(const int fd, chunkqueue * const cq, off_t max_bytes, log_error_st * const errh) {
    while (NULL != cq->first) {
        int rc = -1;
...
#if defined(NETWORK_WRITE_USE_SENDFILE)
int network_write_chunkqueue_sendfile(const int fd, chunkqueue * const cq, off_t max_bytes, log_error_st * const errh) {
    while (NULL != cq->first) {
      #if defined(NETWORK_WRITE_USE_FREEBSD_SENDFILE) \
       || defined(NETWORK_WRITE_USE_DARWIN_SENDFILE) \
       || defined(NETWORK_WRITE_USE_SOLARIS_SENDFILEV)
