...
/* 인라인 디스패치 함수 */
int
send_1xx_dispatch(request_st *r, connection *con) {
    /* HTTP 버전에 따른 최적화된 디스패치 */
    if (r->http_version == HTTP_VERSION_1_1 || r->http_version == HTTP_VERSION_1_0) {
        return h1_send_1xx(r, con);
    }
    if (r->http_version == HTTP_VERSION_2) {
        return h2_send_1xx(r, con);
    }
    
    return 0; /* 지원하지 않는 HTTP 버전 */
}
...
handler_t gw_response_headers_upgrade(request_st *r, struct http_response_opts_t *opts);
handler_t cgi_response_headers(request_st *r, struct http_response_opts_t *opts);
handler_t fcgi_response_headers(request_st *r, struct http_response_opts_t *opts);
handler_t proxy_response_headers(request_st *r, struct http_response_opts_t *opts);

/* 인라인 디스패치 함수 */
static inline handler_t
headers_dispatch(request_st *r, http_response_opts *opts) {
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    handler_t (*func)(request_st *, struct http_response_opts_t *) = opts->headers;
    
    if (func == gw_response_headers_upgrade) {
        return gw_response_headers_upgrade(r, opts);
    }
    if (func == cgi_response_headers) {
        return cgi_response_headers(r, opts);
    }
    if (func == fcgi_response_headers) {
        return fcgi_response_headers(r, opts);
    }
    if (func == proxy_response_headers) {
        return proxy_response_headers(r, opts);
    }
...
handler_t http_response_parse_headers(request_st * const r, http_response_opts * const opts, buffer * const b) {
    ...
    return (opts->headers) ? headers_dispatch(r, opts) : HANDLER_GO_ON;
}

// inlining_added
handler_t ajp13_recv_parse(request_st *r, http_response_opts *opts, buffer *b, size_t n);
handler_t fcgi_recv_parse(request_st *r, http_response_opts *opts, buffer *b, size_t n);

/* 인라인 디스패치 함수 */
static inline handler_t
parse_dispatch(request_st *r, http_response_opts *opts, buffer *b, size_t n) {
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    handler_t (*func)(request_st *, http_response_opts *, buffer *, size_t) = opts->parse;
    
    // if (func == ajp13_recv_parse) {
    //     return ajp13_recv_parse(r, opts, b, n);
    // }
    // if (func == fcgi_recv_parse) {
    //     return fcgi_recv_parse(r, opts, b, n);
    // }
...
handler_t http_response_read(request_st * const r, http_response_opts * const opts, buffer * const b, fdnode * const fdn) {
    ...
    do {
        ...
        if (NULL != opts->parse) {
            handler_t rc = parse_dispatch(r, opts, b, (size_t)n);
            if (rc != HANDLER_GO_ON) return rc;
        } else if (0 == n) {
            if (buffer_is_blank(b))
                chunk_buffer_yield(b); /*(improve large buf reuse)*/
