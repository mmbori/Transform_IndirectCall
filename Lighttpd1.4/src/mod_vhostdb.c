...
int mod_vhostdb_dbi_query(request_st *r, void *p_d, buffer *result);
int mod_vhostdb_ldap_query(request_st *r, void *p_d, buffer *result);
int mod_vhostdb_pgsql_query(request_st *r, void *p_d, buffer *result);
int mod_vhostdb_mysql_query(request_st *r, void *p_d, buffer *result);

/* query 디스패치 함수 */
static inline int
query_dispatch(request_st *r, const http_vhostdb_backend_t *backend, buffer *result) {
    int (*func)(request_st *, void *, buffer *) = backend->query;
    
    // if (func == mod_vhostdb_dbi_query) {
    //     return mod_vhostdb_dbi_query(r, backend->p_d, result);
    // }
    // if (func == mod_vhostdb_ldap_query) {
    //     return mod_vhostdb_ldap_query(r, backend->p_d, result);
    // }
    // if (func == mod_vhostdb_pgsql_query) {
    //     return mod_vhostdb_pgsql_query(r, backend->p_d, result);
    // }
    // if (func == mod_vhostdb_mysql_query) {
    //     return mod_vhostdb_mysql_query(r, backend->p_d, result);
    // }
...
REQUEST_FUNC(mod_vhostdb_handle_docroot) {
    ...
    if (0 != query_dispatch(r, backend->p_d, b)) {
