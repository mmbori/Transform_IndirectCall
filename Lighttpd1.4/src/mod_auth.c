...
void http_auth_cache_entry_free (void *data);
...
handler_t mod_auth_check_basic(request_st *r, void *p_d, const struct http_auth_require_t *require, const struct http_auth_backend_t *backend);
handler_t mod_auth_check_digest(request_st *r, void *p_d, const struct http_auth_require_t *require, const struct http_auth_backend_t *backend);
handler_t mod_auth_check_extern(request_st *r, void *p_d, const struct http_auth_require_t *require, const struct http_auth_backend_t *backend);
...
void data_auth_free(data_unset *d)
{
		...
		for (uint32_t n = 0; n < value->used; ++n) {
			...
			if (require) { /*(always true at this point)*/
				...
				if (!mod_auth_require_parse(dauth->require, require, errh)) {
					free_dispatch((data_unset *)dauth);
					return HANDLER_ERROR;
				}
...
handler_t mod_authn_gssapi_check(request_st *r, void *p_d, const struct http_auth_require_t *require, const struct http_auth_backend_t *backend);
// inlining_added
static inline handler_t
checkfn_dispatch(request_st *r, const http_auth_scheme_t *scheme, const struct http_auth_require_t *require, const struct http_auth_backend_t *backend) {
    if (scheme->checkfn == mod_auth_check_basic) {
        return mod_auth_check_basic(r, scheme->p_d, require, backend);
    }
    if (scheme->checkfn == mod_auth_check_digest) {
        return mod_auth_check_digest(r, scheme->p_d, require, backend);
    }
    if (scheme->checkfn == mod_auth_check_extern) {
        return mod_auth_check_extern(r, scheme->p_d, require, backend);
    }
...
handler_t mod_auth_uri_handler(request_st * const r, void *p_d) {
...

	{
			...
			return checkfn_dispatch(r, scheme->p_d, dauth->require, p->conf.auth_backend);
	}
}
...
// inlining_added
handler_t mod_authn_dbi_basic(request_st *r, void *p_d, const http_auth_require_t *require, const buffer *username, const char *pw);
handler_t mod_authn_file_htdigest_basic(request_st *r, void *p_d, const http_auth_require_t *require, const buffer *username, const char *pw);
handler_t mod_authn_file_htpasswd_basic(request_st *r, void *p_d, const http_auth_require_t *require, const buffer *username, const char *pw);
handler_t mod_authn_file_plain_basic(request_st *r, void *p_d, const http_auth_require_t *require, const buffer *username, const char *pw);
handler_t mod_authn_gssapi_basic(request_st *r, void *p_d, const http_auth_require_t *require, const buffer *username, const char *pw);
handler_t mod_authn_ldap_basic(request_st *r, void *p_d, const http_auth_require_t *require, const buffer *username, const char *pw);
handler_t mod_authn_pam_basic(request_st *r, void *p_d, const http_auth_require_t *require, const buffer *username, const char *pw);
handler_t mod_authn_sasl_basic(request_st *r, void *p_d, const http_auth_require_t *require, const buffer *username, const char *pw);

/* basic 디스패치 함수 */
static inline handler_t
basic_dispatch(request_st *r, const http_auth_backend_t *backend, const http_auth_require_t *require, const buffer *username, const char *pw) {   
    // if (func == mod_authn_dbi_basic) {
    //     return mod_authn_dbi_basic(r, backend->p_d, require, username, pw);
    // }
    if (backend->basic == mod_authn_file_htdigest_basic) {
        return mod_authn_file_htdigest_basic(r, backend->p_d, require, username, pw);
    }
    if (backend->basic == mod_authn_file_htpasswd_basic) {
        return mod_authn_file_htpasswd_basic(r, backend->p_d, require, username, pw);
    }
    if (backend->basic == mod_authn_file_plain_basic) {
        return mod_authn_file_plain_basic(r, backend->p_d, require, username, pw);
    }
    // if (func == mod_authn_gssapi_basic) {
    //     return mod_authn_gssapi_basic(r, backend->p_d, require, username, pw);
    // }
    // if (func == mod_authn_ldap_basic) {
    //     return mod_authn_ldap_basic(r, backend->p_d, require, username, pw);
    // }
    // if (func == mod_authn_pam_basic) {
    //     return mod_authn_pam_basic(r, backend->p_d, require, username, pw);
    // }
    // if (func == mod_authn_sasl_basic) {
    //     return mod_authn_sasl_basic(r, backend->p_d, require, username, pw);
    // }
    return HANDLER_ERROR;
}
...
mod_auth_check_basic(request_st * const r, void *p_d, const struct http_auth_require_t * const require, const struct http_auth_backend_t * const backend)
{
    ...
    *pw++ = '\0';
    ...
    if (NULL == ae) {
        ...
        rc = basic_dispatch(r, backend, require, &userb, pw);
    }
...
mod_auth_digest_mutate (http_auth_info_t * const ai, const http_auth_digest_params_t * const dp, const buffer * const method)
{
    ...
    if (ai->dalgo & HTTP_AUTH_DIGEST_SESS) {
        ...
        MD5_iov(ai->digest, iov, 5);    // digest_iov -> MD5_iov로 수정
        li_tohex(a1, sizeof(a1), (const char *)ai->digest, ai->dlen);
    }
  ...
  #endif
    MD5_iov(ai->digest, iov, n);    // digest_iov -> MD5_iov로 수정
    ...
    MD5_iov(ai->digest, iov, n+1);  // digest_iov -> MD5_iov로 수정
}
...
// inlining_added
handler_t mod_authn_dbi_digest(request_st *r, void *p_d, http_auth_info_t *ai);
handler_t mod_authn_file_htdigest_digest(request_st *r, void *p_d, http_auth_info_t *ai);
handler_t mod_authn_file_plain_digest(request_st *r, void *p_d, http_auth_info_t *ai);

/* digest 디스패치 함수 */
static inline handler_t
digest_dispatch(request_st *r, const http_auth_backend_t *backend, http_auth_info_t *ai) {
    handler_t (*func)(request_st *, void *, http_auth_info_t *) = backend->digest;
    
    // if (func == mod_authn_dbi_digest) {
    //     return mod_authn_dbi_digest(r, backend->p_d, ai);
    // }
    if (func == mod_authn_file_htdigest_digest) {
        return mod_authn_file_htdigest_digest(r, backend->p_d, ai);
    }
    if (func == mod_authn_file_plain_digest) {
        return mod_authn_file_plain_digest(r, backend->p_d, ai);
    }
...
mod_auth_digest_get (request_st * const r, void *p_d, const struct http_auth_require_t * const require, const struct http_auth_backend_t * const backend, http_auth_info_t * const ai)
{
    ...
    if (NULL == ae) {
        ...
        rc = digest_dispatch(r, backend, ai);
    }
...
handler_t mod_auth_check_extern(request_st * const r, void *p_d, const struct http_auth_require_t * const require, const struct http_auth_backend_t * const backend) {
