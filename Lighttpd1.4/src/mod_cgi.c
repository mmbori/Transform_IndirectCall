...
handler_t cgi_handle_fdevent_send (void *ctx, int revents) {
...
handler_t cgi_response_headers(request_st * const r, struct http_response_opts_t *opts) {
...
handler_t cgi_handle_fdevent(void *ctx, int revents) {
	handler_ctx *hctx = ctx;
...
int cgi_env_add(void *venv, const char *key, size_t key_len, const char *val, size_t val_len) {
	env_accum *env = venv;
