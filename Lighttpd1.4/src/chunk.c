...
void stat_cache_entry_refchg(void *ref, int delta);

/* 인라인 디스패치 함수 */
static inline void
file_refchg_dispatch(chunk *c) {
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    void (*func)(void *, int) = c->file.refchg;
    
    if (func == stat_cache_entry_refchg) {
        stat_cache_entry_refchg(c->file.ref, -1);
    }
    /* func == NULL (0)인 경우는 아무것도 하지 않음 */
}
