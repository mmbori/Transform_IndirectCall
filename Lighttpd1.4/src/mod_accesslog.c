...
/* 인라인 디스패치 함수 */
static inline void
esc_fn_dispatch(esc_fn_t esc_fn, buffer * restrict b, const char * restrict s, size_t len) {
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    if (esc_fn == buffer_append_bs_escaped) {
        buffer_append_bs_escaped(b, s, len);
    }
    else if (esc_fn == buffer_append_bs_escaped_json) {
        buffer_append_bs_escaped_json(b, s, len);
    }
}
                         ...
                         const buffer * const restrict b, esc_fn_t esc_fn)
{
    if (!buffer_string_is_empty(b))
        esc_fn_dispatch(esc_fn, dest, BUF_PTR_LEN(b));
    else
                         ...
                         esc_fn_t esc_fn)
{
    ...
    do {
        ...
        if (0 == strncmp(str, name->ptr, len) && str[len] == '=') {
            ...
            esc_fn_dispatch(esc_fn, dest, v, str - v + 1);
            break;
        }
        else {
      ...
      case FORMAT_URL:
        {
            ...
            esc_fn_dispatch(esc_fn, b, r->target.ptr,
                   qmark ? (uint32_t)(qmark - r->target.ptr) : len);
        }
        break;
      case FORMAT_QUERY_STRING:
        esc_fn_dispatch(esc_fn, b, BUF_PTR_LEN(&r->uri.query));
        break;
...
static int log_access_record (const request_st * const r, buffer * const b, format_fields * const parsed_format, esc_fn_t esc) {
	...
	for (const format_field *f = parsed_format->ptr; f->field != FORMAT_UNSET; ++f) {
			...
			case FORMAT_REQUEST_LINE:
				...
				buffer_append_char(b, ' ');
				esc_fn_dispatch(esc, b, BUF_PTR_LEN(&r->target_orig));
				buffer_append_char(b, ' ');
...
handler_t log_access_write(request_st *r, void *p_d) {
    plugin_data * const p = p_d;
