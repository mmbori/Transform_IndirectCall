...
int fcgi_env_add(void *venv, const char *key, size_t key_len, const char *val, size_t val_len) {
	buffer *env = venv;
	char len_enc[8];
...
handler_t fcgi_stdin_append(handler_ctx *hctx) {
	FCGI_Header header;
...
handler_t fcgi_create_env(handler_ctx *hctx) {
...
handler_t fcgi_recv_parse_loop(request_st * const r, handler_ctx * const hctx) {
	/*
...
handler_t fcgi_response_headers(request_st * const r, struct http_response_opts_t *opts) {
