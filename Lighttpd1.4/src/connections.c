...
// inlining_added
int connection_read_cq(connection *con, chunkqueue *cq, off_t max_bytes);
int h2_read_client_connection_preface(connection *con, chunkqueue *cq, off_t max_bytes);
int mod_extforward_network_read(connection *con, chunkqueue *cq, off_t max_bytes);
int server_oneshot_read_cq(connection *con, chunkqueue *cq, off_t max_bytes);

// 최적화된 network_read 디스패처 함수
static inline int
network_read_dispatch(connection * const con, chunkqueue * const cq, off_t max_bytes)
{
    /* 함수 포인터 주소를 직접 비교해서 해당 함수를 직접 호출 */
    if (con->network_read == connection_read_cq) {
        return connection_read_cq(con, cq, max_bytes);
    }
    else if (con->network_read == h2_read_client_connection_preface) {
        return h2_read_client_connection_preface(con, cq, max_bytes);
    }
    else if (con->network_read == mod_extforward_network_read) {
        return mod_extforward_network_read(con, cq, max_bytes);
    }
    else if (con->network_read == server_oneshot_read_cq) {
        return server_oneshot_read_cq(con, cq, max_bytes);
    }
}

static void connection_read_for_eos_ssl(connection * const con) {
	if (network_read_dispatch(con, con->read_queue, MAX_READ_LIMIT) < 0)
		con->close_timeout_ts = log_monotonic_secs - (HTTP_LINGER_TIMEOUT+1);
	chunkqueue_reset(con->read_queue);
}
...
int connection_write_cq(connection *con, chunkqueue *cq, off_t max_bytes);
...
connection_write_chunkqueue (connection * const con, chunkqueue * const restrict cq, off_t max_bytes)
{
    ...
    ret = connection_write_cq(con, cq, max_bytes);
...
__attribute_noinline__
int connection_handle_write(request_st * const r, connection * const con) {
	/*assert(!chunkqueue_is_empty(cq));*//* checked by callers */
...
// inlining_added
handler_t mod_cgi_handle_subrequest(request_st *r, void *p_d);
// handler_t mod_magnet_handle_subrequest(request_st *r, void *p_d);
// handler_t mod_webdav_subrequest_handler(request_st *r, void *p_d);
handler_t gw_handle_subrequest(request_st *r, void *p_d);

/* 인라인 디스패치 함수 */
static inline handler_t
handle_subrequest_dispatch(request_st *r, const plugin *p) {
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    handler_t (*func)(request_st *, void *) = p->handle_subrequest;
    
    if (func == gw_handle_subrequest) {
        return gw_handle_subrequest(r, p->data);
    }
    if (func == mod_cgi_handle_subrequest) {
        return mod_cgi_handle_subrequest(r, p->data);
    }
    // if (func == mod_magnet_handle_subrequest) {
    //     return mod_magnet_handle_subrequest(r, p->data);
    // }
    // if (func == mod_webdav_subrequest_handler) {
    //     return mod_webdav_subrequest_handler(r, p->data);
    // }
...
static int connection_handle_write_state(request_st * const r, connection * const con) {
    ...
    do {
        ...
        if (r->handler_module && !r->resp_body_finished) {
            ...
            if (handle_subrequest_dispatch(r, p->data) > HANDLER_WAIT_FOR_EVENT) {
                connection_set_state_error(r, CON_STATE_ERROR);
                return CON_STATE_ERROR;
            }
        }
...
handler_t connection_handle_fdevent(void * const context, const int revents) {
...
// inlining_added
int network_write_chunkqueue_sendfile(int fd, chunkqueue *cq, off_t max_bytes, log_error_st *errh);
int network_write_chunkqueue_writev(int fd, chunkqueue *cq, off_t max_bytes, log_error_st *errh);

/* 인라인 디스패치 함수 */
static inline int
network_backend_write_dispatch(server *srv, int fd, chunkqueue *cq, off_t max_bytes, log_error_st *errh) {
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    int (*func)(int, chunkqueue *, off_t, log_error_st *) = srv->network_backend_write;
    
    if (func == network_write_chunkqueue_sendfile) {
        return network_write_chunkqueue_sendfile(fd, cq, max_bytes, errh);
    }
    if (func == network_write_chunkqueue_writev) {
        return network_write_chunkqueue_writev(fd, cq, max_bytes, errh);
    }
...
int connection_write_cq(connection *con, chunkqueue *cq, off_t max_bytes) {
    request_st * const r = &con->request;
    return network_backend_write_dispatch(con->srv, con->fd,cq,max_bytes,r->conf.errh);
}
...
int h2_process_streams (connection * const con, handler_t(*http_response_loop)(request_st *), int(*connection_handle_write)(request_st *, connection *));

void
connection_state_machine (connection * const con)
{
    int rc = !con->fn || h2_process_streams(con, http_response_handler,
...
int h2_send_goaway_graceful (connection * const con);
