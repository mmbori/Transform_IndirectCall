...
handler_t gw_handle_fdevent(void *ctx, int revents);
...
// inlining_added
handler_t ajp13_stdin_append(gw_handler_ctx *hctx);
handler_t fcgi_stdin_append(gw_handler_ctx *hctx);
handler_t proxy_stdin_append(gw_handler_ctx *hctx);

/* 인라인 디스패치 함수 */
static inline handler_t
stdin_append_dispatch(gw_handler_ctx *hctx) {
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    handler_t (*func)(gw_handler_ctx *) = hctx->stdin_append;
    
    if (func == ajp13_stdin_append) {
        return ajp13_stdin_append(hctx);
    }
    if (func == fcgi_stdin_append) {
        return fcgi_stdin_append(hctx);
    }
    if (func == proxy_stdin_append) {
        return proxy_stdin_append(hctx);
    }
...
static handler_t gw_write_refill_wb(gw_handler_ctx * const hctx, request_st * const r) {
    ...
    if (hctx->stdin_append) {
        if (chunkqueue_length(&hctx->wb) < 65536 - 16384)
            return stdin_append_dispatch(hctx);
    }
    else {
...
// inlining_added
handler_t ajp13_create_env(gw_handler_ctx *hctx);
handler_t fcgi_create_env(gw_handler_ctx *hctx);
handler_t scgi_create_env(gw_handler_ctx *hctx);
handler_t sockproxy_create_env_connect(gw_handler_ctx *hctx);

static  handler_t
create_env_dispatch(gw_handler_ctx *hctx) {
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    if (hctx->create_env == ajp13_create_env) {
        return ajp13_create_env(hctx);
    }
    if (hctx->create_env == fcgi_create_env) {
        return fcgi_create_env(hctx);
    }
    if (hctx->create_env == scgi_create_env) {
        return scgi_create_env(hctx);
    }
    if (hctx->create_env == sockproxy_create_env_connect) {
        return sockproxy_create_env_connect(hctx);
    }
...
static handler_t gw_write_request(gw_handler_ctx * const hctx, request_st * const r) {
    ...
    case GW_STATE_PREPARE_WRITE:
        ...
        {
            handler_t rc = create_env_dispatch(hctx);
            if (HANDLER_GO_ON != rc) {
    ...
    case GW_STATE_WRITE:
        if (!chunkqueue_is_empty(&hctx->wb)) {
            ...
            if (network_backend_write_dispatch(r->con->srv, hctx->fd, &hctx->wb,
                                                   MAX_WRITE_LIMIT,
                                                   r->conf.errh) < 0) {
...
void wstunnel_backend_error(gw_handler_ctx *gwhctx);

__attribute_cold__
__attribute_noinline__
static handler_t gw_backend_error(gw_handler_ctx * const hctx, request_st * const r)
{
    // if (hctx->backend_error) wstunnel_backend_error(hctx);
...
handler_t h2_recv_reqbody(request_st *r);

/* 인라인 디스패치 함수 */
static inline handler_t
reqbody_read_dispatch(request_st *r) {
    /* 함수 포인터 직접 비교를 통한 최적화된 디스패치 */
    handler_t (*func)(request_st *) = r->con->reqbody_read;
    
    if (func == h1_reqbody_read) {
        return h1_reqbody_read(r);
    }
    if (func == h2_recv_reqbody) {
        return h2_recv_reqbody(r);
    }
    return HANDLER_ERROR;
}
...
handler_t gw_handle_subrequest(request_st * const r, void *p_d) {
    ...
    if (hctx->gw_mode != GW_AUTHORIZER
        ...
        if (chunkqueue_length(&hctx->wb) > 65536 - 4096
            ...
            handler_t rc = reqbody_read_dispatch(r);
            ...
            if (hctx->wb_reqlen < -1 && r->reqbody_length >= 0) {
                ...
                if (hctx->stdin_append) {
                    handler_t rca = stdin_append_dispatch(hctx);
...
handler_t gw_response_headers_upgrade(request_st * const r, struct http_response_opts_t *opts) {
