...
// inlining_added
extern int cgi_env_add(void *venv, const char *key, size_t key_len, const char *val, size_t val_len);
extern int fcgi_env_add(void *venv, const char *key, size_t key_len, const char *val, size_t val_len);
extern int scgi_env_add_scgi(void *venv, const char *key, size_t key_len, const char *val, size_t val_len);
extern int scgi_env_add_uwsgi(void *venv, const char *key, size_t key_len, const char *val, size_t val_len);
extern int ssi_env_add(void *venv, const char *key, size_t key_len, const char *val, size_t val_len);
...
// inlining_added
static inline int
optimized_callback_dispatch(http_cgi_header_append_cb cb, void *vdata,
                           const char *key_ptr, size_t key_len,
                           const char *val_ptr, size_t val_len)
{
    /* 함수 포인터 주소를 직접 비교해서 해당 함수를 직접 호출 */
    if (cb == cgi_env_add) {
        return cgi_env_add(vdata, key_ptr, key_len, val_ptr, val_len);
    }
    else if (cb == fcgi_env_add) {
        return fcgi_env_add(vdata, key_ptr, key_len, val_ptr, val_len);
    }
    else if (cb == scgi_env_add_scgi) {
        return scgi_env_add_scgi(vdata, key_ptr, key_len, val_ptr, val_len);
    }
    else if (cb == scgi_env_add_uwsgi) {
        return scgi_env_add_uwsgi(vdata, key_ptr, key_len, val_ptr, val_len);
    }
    else if (cb == ssi_env_add) {
        return ssi_env_add(vdata, key_ptr, key_len, val_ptr, val_len);
    }
}
...
http_cgi_headers (request_st * const r, http_cgi_opts * const opts, http_cgi_header_append_cb cb, void *vdata)
{
    ...
    if (!opts->authorizer) {
        rc |= optimized_callback_dispatch(cb, vdata, CONST_STR_LEN("CONTENT_LENGTH"),
                 buf, li_itostrn(buf,sizeof(buf),r->reqbody_length));
    }

    n = buffer_clen(&r->uri.query);
    rc |= optimized_callback_dispatch(cb, vdata, CONST_STR_LEN("QUERY_STRING"),
                    n ? r->uri.query.ptr : "", n);
    ...
    rc |= optimized_callback_dispatch(cb, vdata, CONST_STR_LEN("REQUEST_URI"), s+len, n-len);

    if (!buffer_is_equal(&r->target, &r->target_orig))
        rc |= optimized_callback_dispatch(cb, vdata, CONST_STR_LEN("REDIRECT_URI"),
    ...
    if (0 == r->error_handler_saved_status)
        rc |= optimized_callback_dispatch(cb, vdata, CONST_STR_LEN("REDIRECT_STATUS"),
                        CONST_STR_LEN("200"));
     ...
     */
    if (!opts->authorizer) {
        rc |= optimized_callback_dispatch(cb, vdata, CONST_STR_LEN("SCRIPT_NAME"),
                        BUF_PTR_LEN(&r->uri.path));
        if (!buffer_is_blank(&r->pathinfo)) {
            rc |= optimized_callback_dispatch(cb, vdata, CONST_STR_LEN("PATH_INFO"),
            ...
            buffer_copy_path_len2(tb, BUF_PTR_LEN(bd),
                                      BUF_PTR_LEN(&r->pathinfo));
            rc |= optimized_callback_dispatch(cb, vdata, CONST_STR_LEN("PATH_TRANSLATED"),
                            BUF_PTR_LEN(tb));
        }
    }
    ...
    */
    ...
    if (opts->docroot) {
        ...
        buffer_copy_path_len2(tb, BUF_PTR_LEN(opts->docroot),
                                  BUF_PTR_LEN(&r->uri.path));
        rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("SCRIPT_FILENAME"),
                        BUF_PTR_LEN(tb));
        rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("DOCUMENT_ROOT"),
    ...
    if (!r->h2_connect_ext) {
        ...
        rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("REQUEST_METHOD"), BUF_PTR_LEN(m));

        const buffer * const v = http_version_buf(r->http_version);
        rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("SERVER_PROTOCOL"), BUF_PTR_LEN(v));
    }
    else {
        /*(SERVER_PROTOCOL=HTTP/1.1 instead of HTTP/2.0)*/
        rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("REQUEST_METHOD"),
                        CONST_STR_LEN("GET"));
        rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("SERVER_PROTOCOL"),
        ...
        if (!http_header_request_get(r, HTTP_HEADER_OTHER,
            ...
            rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("HTTP_SEC_WEBSOCKET_KEY"),
                            CONST_STR_LEN("MDAwMDAwMDAwMDAwMDAwMA=="));
        /*(Upgrade and Connection should not exist for HTTP/2 request)*/
        rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("HTTP_UPGRADE"),
                        CONST_STR_LEN("websocket"));
        rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("HTTP_CONNECTION"),
    ...
    rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("SERVER_SOFTWARE"), s, n);

    rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("GATEWAY_INTERFACE"),
                    CONST_STR_LEN("CGI/1.1"));

    rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("REQUEST_SCHEME"),
                    BUF_PTR_LEN(&r->uri.scheme));

    if (buffer_is_equal_string(&r->uri.scheme, CONST_STR_LEN("https")))
        rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("HTTPS"), CONST_STR_LEN("on"));
    ...
    rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("SERVER_PORT"), s, n);
    ...
    rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("SERVER_ADDR"), s, n);
    ...
    rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("SERVER_NAME"), s, n);

    rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("REMOTE_ADDR"),
                    BUF_PTR_LEN(r->dst_addr_buf));

    rc |= optimized_callback_dispatch(cb,vdata, CONST_STR_LEN("REMOTE_PORT"), buf,
    ...
    for (n = 0; n < r->rqst_headers.used; n++) {
        ...
        if (!buffer_is_blank(&ds->value) && !buffer_is_unset(&ds->key)) {
            ...
            rc |= optimized_callback_dispatch(cb,vdata, BUF_PTR_LEN(tb),
                            BUF_PTR_LEN(&ds->value));
        }
    }

    plugins_call_handle_request_env(r);
