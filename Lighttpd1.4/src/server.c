...
static inline int
server_oneshot_read_cq_inlining(connection * const con, chunkqueue * const cq, off_t max_bytes)
{
...
int server_oneshot_read_cq(connection *con, chunkqueue *cq, off_t max_bytes) {
    ...
    int rc = server_oneshot_read_cq_inlining(con, cq, max_bytes);
    con->fd = fd;
...
server_oneshot_handle_fdevent_inlining(fdevent_handler handler, void *context, int revents)
{
    /* 함수 포인터 주소를 직접 비교해서 해당 함수를 직접 호출 */
...
handler_t server_oneshot_handle_fdevent(void *context, int revents) {
    ...
    handler_t rc = (fdn && (fdevent_handler)NULL != fdn->handler)
      ? server_oneshot_handle_fdevent_inlining(fdn->handler, con, revents)
      : HANDLER_FINISHED;
