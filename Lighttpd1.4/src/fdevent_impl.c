...
// inlining added
void fdevent_linux_sysepoll_free(fdevents *ev);
// void fdevent_freebsd_kqueue_free(fdevents *ev);
// void fdevent_solaris_port_free(fdevents *ev);
// void fdevent_solaris_devpoll_free(fdevents *ev);
void fdevent_poll_free(fdevents *ev);

// // inlining added
// int fdevent_select_reset (fdevents *ev);
// int fdevent_freebsd_kqueue_init (fdevents *ev);
// int fdevent_solaris_devpoll_reset (fdevents *ev);

// inlining added
int fdevent_linux_sysepoll_poll (fdevents *ev, int timeout_ms);
// int fdevent_freebsd_kqueue_poll (fdevents *ev, int timeout_ms);
// int fdevent_solaris_port_poll (fdevents *ev, int timeout_ms);
// int fdevent_solaris_devpoll_poll (fdevents *ev, int timeout_ms);

// inlining_added
handler_t connection_handle_fdevent(void *context, int revents);
handler_t gw_handle_fdevent(void *context, int revents);
handler_t cgi_handle_fdevent_send(void *context, int revents);
handler_t cgi_handle_fdevent(void *context, int revents);
handler_t network_server_handle_fdevent(void *context, int revents);
handler_t server_oneshot_handle_fdevent(void *context, int revents);
handler_t stat_cache_handle_fdevent(void *context, int revents);

// 최적화된 fdevent_handler 디스패처 함수
static inline handler_t
fnd_handler_dispatch(fdevent_handler handler, void *context, int revents)
{
    /* 함수 포인터 주소를 직접 비교해서 해당 함수를 직접 호출 */
    if (handler == connection_handle_fdevent) {
        return connection_handle_fdevent(context, revents);
    }
    else if (handler == gw_handle_fdevent) {
        return gw_handle_fdevent(context, revents);
    }
    else if (handler == cgi_handle_fdevent_send) {
        return cgi_handle_fdevent_send(context, revents);
    }
    else if (handler == cgi_handle_fdevent) {
        return cgi_handle_fdevent(context, revents);
    }
    else if (handler == network_server_handle_fdevent) {
        return network_server_handle_fdevent(context, revents);
    }
    else if (handler == server_oneshot_handle_fdevent) {
        return server_oneshot_handle_fdevent(context, revents);
    }
    else if (handler == stat_cache_handle_fdevent) {
        return stat_cache_handle_fdevent(context, revents);
    }
}
...
// inlining added
static inline void
fdevent_free_dispatch(fdevents *ev)
{
    /* 함수 포인터 주소를 직접 비교해서 해당 함수를 직접 호출 */
    if (ev->free == fdevent_linux_sysepoll_free) {
        fdevent_linux_sysepoll_free(ev);
    }
    // else if (ev->free == fdevent_freebsd_kqueue_free) {
    //     fdevent_freebsd_kqueue_free(ev);
    // }
    // else if (ev->free == fdevent_solaris_port_free) {
    //     fdevent_solaris_port_free(ev);
    // }
    // else if (ev->free == fdevent_solaris_devpoll_free) {
    //     fdevent_solaris_devpoll_free(ev);
    // }
    else if (ev->free == fdevent_poll_free) {
        fdevent_poll_free(ev);
    }
}
...
{
    if (!ev) return;
    if (ev->free) fdevent_free_dispatch(ev);
...
// // inlining added
// static int
// fdevent_reset_dispatch(fdevents *ev)
// {
//     /* 함수 포인터 주소를 직접 비교해서 해당 함수를 직접 호출 */
//     if (ev->free == fdevent_select_reset) {
//         fdevent_select_reset(ev);
//     }
//     // else if (ev->free == fdevent_freebsd_kqueue_init) {
//     //     fdevent_freebsd_kqueue_init(ev);
//     // }
//     else if (ev->free == fdevent_solaris_devpoll_reset) {
//         fdevent_solaris_devpoll_reset(ev);
//     }
// }
...
// inlining added
int fdevent_poll_dispatch(fdevents *ev, int timeout_ms) {
    /* 함수 포인터 주소를 직접 비교해서 해당 함수를 직접 호출 */
    if (ev->poll == fdevent_linux_sysepoll_poll) {
        fdevent_linux_sysepoll_poll(ev, timeout_ms);
    }
    // else if (ev->poll == fdevent_freebsd_kqueue_poll) {
    //     fdevent_freebsd_kqueue_poll(ev, timeout_ms);
    // }
    // else if (ev->poll == fdevent_solaris_port_poll) {
    //     fdevent_solaris_port_poll(ev, timeout_ms);
    // }
    // else if (ev->poll == fdevent_solaris_devpoll_poll) {
    //     fdevent_solaris_devpoll_poll(ev, timeout_ms);
    // }
}
...
{
    const int n = fdevent_poll_dispatch(ev, ev->pendclose ? 0 : timeout_ms);
    if (n >= 0)
...
int fdevent_linux_sysepoll_poll (fdevents * const ev, int timeout_ms)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if ((fdevent_handler)NULL != fdn->handler)
            fnd_handler_dispatch(fdn->handler, fdn->ctx, revents);
    }
    return n;
}
...
fdevent_poll_poll (fdevents *ev, int timeout_ms)
{
    ...
    for (int m = 0, a = 0; m < n && a < count; ++a) {
        ...
        for (int i = 0; i < nfds; ++i) {
            ...
            if (0 == ((uintptr_t)fdn & 0x3))
                fnd_handler_dispatch(fdn->handler, fdn->ctx, pfds[i].revents);
            ++m;
            break;
        }
    }
...
int fdevent_select_event_del (fdevents *ev, fdnode *fdn)
{
  #ifdef _WIN32
