...
h2_read_client_connection_preface (struct connection * const con, chunkqueue * const cq, off_t max_bytes)
{
    ...
    int rc = network_read_dispatch(con, cq, max_bytes);
...
void h2_init_con (request_st * const restrict h2r, connection * const restrict con); 
...
h2_retire_con (request_st * const h2r, connection * const con)
{
    ...
    if (h2r->state != CON_STATE_ERROR) { /*(CON_STATE_RESPONSE_END)*/
        ...
        if (!chunkqueue_is_empty(con->write_queue)) {
            ...
            connection_write_cq(con, cq, len);
            /*(optional accounting)*/
...
h2_process_streams (connection * const con,
                    ...
                    int(*connection_handle_write_fp)(request_st *, connection *))
{
    h2con * const h2c = (h2con *)con->hx;
    ...
    if (h2c->sent_goaway <= 0
        ...
        if (0 == network_read_dispatch(con, cq, MAX_READ_LIMIT)) {
            if (mark < cq->bytes_in)
                h2_parse_frames(con);
        }
        else {
        ...
        for (uint32_t i = 0; i < h2c->rused; ++i) {
            ...
            switch (r->state) {
              ...
              case CON_STATE_HANDLE_REQUEST:
                {
                    const handler_t rc = http_response_handler(r);
              ...
              case CON_STATE_WRITE:
                ...
                if (r->handler_module && !r->resp_body_finished) {
                    ...
                    if (handle_subrequest_dispatch(r, p->data)
                        > HANDLER_WAIT_FOR_EVENT) {
