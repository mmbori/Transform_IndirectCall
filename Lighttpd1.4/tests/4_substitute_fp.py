...
# """
    ...
    def _convert_to_add(self, callsite, callees, indent_level: int = 0) :
        try:
            with open(f'solve_loop/{callsite}.txt', 'r', encoding='utf-8') as f:
                content = f.read()

            lines = content.strip().split('\n')
            base_indent = '    ' * indent_level  # 4 spaces per level
            formatted_lines = []
            pattern = r'([a-zA-Z0-9_]\w*)\s*->\s*([a-zA-Z0-9_]\w*)\s*\('
            ...
            for callee in callees :
                for line in lines:
                    ...
                    if cleaned_line:
                        if '//' in cleaned_line :
                            continue 
                        replaced_line = re.sub(pattern, f'{callee}(', cleaned_line)
                        formatted_lines.append(f"+ {replaced_line}")
                    else:
    ...
    def _convert_to_remove(self, callsite, indent_level: int = 0) :
        try:
            with open(f'solve_loop/{callsite}.txt', 'r', encoding='utf-8') as f:
                content = f.read()
...
#         """Generate rule for direct transformation in the callsite function"""
#         body_to_remove = self._convert_to_remove(callsite)
#         body_to_add = self._convert_to_add(callsite, callees)
#         rule_name = f"substitute_indirectcall_{callsite}"
...
# ...
# }
# """.replace("{CALLSITE}", callsite).replace("{RULE_NAME}", rule_name).replace("{BODY_TO_REMOVE}",body_to_remove).replace("{BODY_TO_ADD}", body_to_add)
...
#         return "\n".join([p1])
    ...
    def _generate_function_inlining_rule(self, callsite_caller: str, callsite: str, src_line: str, callees: List[str]) -> str:
        ...
        body_to_remove = self._convert_to_remove(callsite_caller)
        body_to_add = self._convert_to_add(callsite_caller, callees)
        rule_name = f"substitute_indirectcall_{callsite_caller}"
...
""".replace("{CALLER}", callsite_caller).replace("{RULE_NAME}", rule_name).replace("{BODY_TO_REMOVE}",body_to_remove).replace("{BODY_TO_ADD}", body_to_add)
