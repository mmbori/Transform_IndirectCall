...
handler_t plugins_call_handle_physical(request_st * const r) {
	return plugins_call_fn_req_data(r, PLUGIN_FUNC_HANDLE_PHYSICAL);
}
...
handler_t plugins_call_handle_connection_shut_wr(connection *con) {
	return plugins_call_fn_con_data(con, PLUGIN_FUNC_HANDLE_CONNECTION_SHUT_WR);
}
...
handler_t plugins_call_set_defaults(server *srv) {
    const int e = PLUGIN_FUNC_SET_DEFAULTS;
    const unsigned int offset = ((const unsigned short *)srv->plugin_slots)[e];
    if (0 == offset)
        return HANDLER_GO_ON;
    const plugin_fn_srv_data *plfd = (const plugin_fn_srv_data *)(((unsigned long)srv->plugin_slots) + offset);
    handler_t rc = HANDLER_GO_ON;
    while (plfd->fn && (rc = plfd->fn(srv, plfd->data)) == HANDLER_GO_ON)
        ++plfd;
    return rc;
