...
#     cur_callees = None   # set()
#     cur_srcs = None      # list of src_line strings

#     sessions = []  # [{callsite, caller, callees:set, srcs:list}]
...
#                 sl = kv.get('src_line') or kv.get('srv_line') or ''
#                 if sl:
#                     cur_srcs.append(sl)
#                 continue
...
#             if m and in_session:
#                 if not cur_callees:
#                     cur_callees.add("")  # 세션 내 호출 없으면 [""] 유지
#                 sessions.append({
#                     "callsite": cur_cs or "",
#                     "caller": cur_caller or "",
#                     "callees": set(cur_callees),
...
#     # callsite+caller 단위로 합집합/카운터
#     agg = {}  # (cs, caller) -> {"callees": set(), "src_counter": Counter()}
#     for s in sessions:
#         key = (s["callsite"], s["caller"])
#         if key not in agg:
#             agg[key] = {"callees": set(), "src_counter": Counter()}
#         agg[key]["callees"].update(s["callees"])
#         agg[key]["src_counter"].update(s["srcs"])

#     def mk_item(callsite_caller: str, callsite: str, src_counter: Counter, callees_set: set):
#         # 원하는 키 순서대로 삽입
#         item = {}
...
#         if not callees_set:
#             callees_set = {""}
#         item["callee"] = sorted(callees_set)
#         return item

#     out_items = []

#     # (a) 비어있지 않은 caller가 1종류 이하인 callsite → 단일 항목(caller="")
#     for cs, caller_set in nonempty_callers_by_cs.items():
#         if len(caller_set) <= 1:
#             merged_callees = set()
#             merged_src_counter = Counter()
#             for (k_cs, _caller), payload in agg.items():
#                 if k_cs == cs:
#                     merged_callees.update(payload["callees"])
#                     merged_src_counter.update(payload["src_counter"])
#             out_items.append(mk_item("", cs, merged_src_counter, merged_callees))

#     # (b) 다중 호출자: caller별로 항목 1개씩
#     for cs, caller_set in nonempty_callers_by_cs.items():
#         if len(caller_set) >= 2:
#             for caller in sorted(caller_set):
#                 payload = agg.get((cs, caller), {"callees": set(), "src_counter": Counter()})
#                 out_items.append(mk_item(caller, cs, payload["src_counter"], payload["callees"]))

#     # (c) 전부 빈 caller만 있는 callsite
#     only_empty_callers_cs = {cs for (cs, _caller) in agg.keys()
#                              if cs not in nonempty_callers_by_cs}
#     for cs in sorted(only_empty_callers_cs):
#         merged_callees = set()
#         merged_src_counter = Counter()
#         for (k_cs, _caller), payload in agg.items():
#             if k_cs == cs:
