...
def find_function_pointers_in_struct_content(struct_content: str, verbose: bool = False) -> List[Tuple[str, str, str]]:
    """구조체 내용에서 함수 포인터 찾기 - 조건부 컴파일과 주석 무시"""
    function_pointers = []
    
    # 강화된 함수 포인터 패턴들
    fp_patterns = [
        # 기본 패턴: return_type (*name)(args);
        re.compile(r'(\w+(?:\s*\*)*|void|unsigned\s+\w+|signed\s+\w+|const\s+\w+(?:\s*\*)*|volatile\s+\w+)\s*\(\s*\*\s*(\w+)\s*\)\s*\([^;{}]*?\)\s*;', re.DOTALL | re.MULTILINE),
        
        # typedef 함수 포인터: typedef_name name;
        re.compile(r'(sqlite3_xauth|sqlite3_callback|sqlite3_exec_callback)\s+(\w+)\s*;', re.MULTILINE),
        
        # 복잡한 타입: struct type (*name)(args);
        re.compile(r'(struct\s+\w+\s*\*?|\w+\s*\*+)\s*\(\s*\*\s*(\w+)\s*\)\s*\([^;{}]*?\)\s*;', re.MULTILINE),
    ]
    
    for i, pattern in enumerate(fp_patterns):
        for match in pattern.finditer(struct_content):
            if i == 1:  # typedef 패턴
                ...
                try:
                    ...
                    args_match = re.search(r'\([^;{}]*?\)\s*\([^;{}]*?\)', full_match)
                    args = args_match.group(0) if args_match else ""
                except:
                    args = ""
            
            if fp_name and len(fp_name) > 1 and fp_name.isidentifier():
                # 중복 확인
                if not any(existing[1] == fp_name for existing in function_pointers):
                    function_pointers.append((return_type, fp_name, args))
                    if verbose:
                        print(f"    패턴{i+1}: {fp_name} ({return_type})")
...
def find_function_pointer_assignments(source_dir: str, fp_names: Set[str], verbose: bool = False) -> Dict[str, Set[str]]:
    ...
    for fp_name in fp_names:
        ...
        # 5. 포인터 멤버 할당: obj1.obj2->fp_name = func_name
        assignment_patterns.append((
            fp_name,
            re.compile(rf'\w+.\w+->{re.escape(fp_name)}\s*=\s*&?(\w+)\s*[;,]', re.MULTILINE)
        ))

        # 6. 포인터 멤버 할당: obj1->obj2.fp_name = func_name
        assignment_patterns.append((
            fp_name,
            re.compile(rf'\w+->\w+.{re.escape(fp_name)}\s*=\s*&?(\w+)\s*[;,]', re.MULTILINE)
        ))
...
def save_results(fp_assignments: Dict[str, Set[str]], func_declarations: Dict[str, str], verbose: bool = False):
    """결과를 파일로 저장"""
    # fpName 디렉토리 생성
    os.makedirs("fpName_new", exist_ok=True)
    os.makedirs("fpNameDecl_new", exist_ok=True)
    ...
    for fp_name, func_names in fp_assignments.items():
        if func_names:
            with open(f"fpName_new/{fp_name}.txt", 'w', encoding='utf-8') as f:
                for func_name in sorted(func_names):
                    f.write(f"{func_name}\n")
            
            if verbose:
                print(f"[SAVED] fpName_new/{fp_name}.txt: {len(func_names)}개 함수")
    ...
    for fp_name, func_names in fp_assignments.items():
        if func_names:
            ...
            if declarations:
                with open(f"fpNameDecl_new/{fp_name}.txt", 'w', encoding='utf-8') as f:
                    f.write(f"// Function declarations for {fp_name}\n\n")
                    for decl in declarations:
                        f.write(f"{decl}\n")
                
                if verbose:
                    print(f"[SAVED] fpNameDecl_new/{fp_name}.txt: {len(declarations)}개 선언")
...
def main():
    ...
    print(f"\n개선사항:")
    print(f"  ✓ 조건부 컴파일 블록 내부 함수 포인터 인식")
    print(f"  ✓ Union 내부 함수 포인터 인식")
    print(f"  ✓ typedef 함수 포인터 타입 인식")
    print(f"  ✓ 원본에서 직접 처리로 정확성 향상")
    
    print(f"\n생성된 파일:")
    print(f"  fpName_new/: 각 함수 포인터별 할당된 함수 목록")
    print(f"  fpNameDecl_new/: 각 함수 포인터별 함수 선언 모음")
