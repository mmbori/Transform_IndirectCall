...
def generate_return_transformation_rules_arrow(target_fp_names: set[str], fpnames_dir: str = "fpName") -> str:
    ...
    for fp_name in sorted(target_fp_names):
        ...
        for i, candi_func in enumerate(candi_funcs):
            if i == 0:
                out.append(f"+ if (memcmp(E->{fp_name}_signature, {fp_name}_signatures[{fp_name}_{candi_func}_enum], sizeof(E->{fp_name}_signature)) == 0) {{\n")
            else:
                out.append(f"+ }}\n")
                out.append(f"+ else if (memcmp(E->{fp_name}_signature, {fp_name}_signatures[{fp_name}_{candi_func}_enum], sizeof(E->{fp_name}_signature)) == 0) {{\n")
            
            out.append(f"+ return {candi_func}(args);\n")
        
        # Close the final if-else chain
        out.append(f"+ }}\n")
        out.append(f"+ else {{\n")
        out.append(f"+ return {candi_funcs[0]}(args);  // Default fallback\n")
        out.append(f"+ }}\n\n")
...
def generate_assignment_transformation_rules_arrow(target_fp_names: set[str], fpnames_dir: str = "fpName") -> str:
    ...
    for fp_name in sorted(target_fp_names):
        ...
        for i, candi_func in enumerate(candi_funcs):
            if i == 0:
                out.append(f"+ if (memcmp(E2->{fp_name}_signature, {fp_name}_signatures[{fp_name}_{candi_func}_enum], sizeof(E2->{fp_name}_signature)) == 0) {{\n")
            else:
                out.append(f"+ }}\n")
                out.append(f"+ else if (memcmp(E2->{fp_name}_signature, {fp_name}_signatures[{fp_name}_{candi_func}_enum], sizeof(E2->{fp_name}_signature)) == 0) {{\n")
