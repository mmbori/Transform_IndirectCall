...
struct RtreeConstraint {
  ...
  union {
    ...
    int xGeom_signature[4];

    int xQueryFunc_signature[4];
  } u;
    ...
    for(i=0; i<pCsr->nConstraint; i++){
      ...
      if( pInfo ){
        if( pInfo->xDelUser ) pInfo->xDelUser(pInfo->pUser);
          // if (memcmp(pInfo->xDelUser_signature, xDelUser_signatures[xDelUser_circle_del_enum], sizeof(pInfo->xDelUser_signature)) == 0) {
          //   circle_del(pInfo->pUser);
          // }
          // if (memcmp(pInfo->xDelUser_signature, xDelUser_signatures[xDelUser_cube_context_free_enum], sizeof(pInfo->xDelUser_signature)) == 0) {
          //   cube_context_free(pInfo->pUser);
          // }
        sqlite3_free(pInfo);
      }
    }
