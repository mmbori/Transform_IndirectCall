  ...
  *rc = multiplexSubFilename(pGroup, iChunk);
  if( (*rc)==SQLITE_OK && (pSubOpen = pGroup->aReal[iChunk].p)==0 ){
    ...
    if( createFlag ){
      ...
      if (memcmp(pOrigVfs->xAccess_signature, xAccess_signatures[xAccess_multiplexAccess_enum], sizeof(pOrigVfs->xAccess_signature)) == 0) {
        *rc = multiplexAccess(pOrigVfs, pGroup->aReal[iChunk].z,
    ...
    if (memcmp(pOrigVfs->xOpen_signature, xOpen_signatures[xOpen_multiplexOpen_enum], sizeof(pOrigVfs->xOpen_signature)) == 0) {
      *rc = multiplexOpen(pOrigVfs, pGroup->aReal[iChunk].z, pSubOpen, flags,
                          pOutFlags);
    }
...
static sqlite3_int64 multiplexSubSize(
  ...
  if( pSub==0 ) return 0;
  if (memcmp(pSub->pMethods->xFileSize_signature, xFileSize_signatures[xFileSize_multiplexFileSize_enum], sizeof(pSub->pMethods->xFileSize_signature)) == 0) {
    *rc = multiplexFileSize(pSub, &sz);
  }
...
static void multiplexSubClose(
  ...
  if( pSubOpen ){
    if (memcmp(pSubOpen->pMethods->xClose_signature, xClose_signatures[xClose_multiplexClose_enum], sizeof(pSubOpen->pMethods->xClose_signature)) == 0) {
      multiplexClose(pSubOpen);
    }
  ...
  */
  ...
  if( rc==SQLITE_OK ){
    ...
    if( rc==SQLITE_OK ){
      ...
      if (memcmp(pSubOpen->pMethods->xFileSize_signature, xFileSize_signatures[xFileSize_multiplexFileSize_enum], sizeof(pSubOpen->pMethods->xFileSize_signature)) == 0) {
        rc = multiplexFileSize(pSubOpen, &sz64);
      }
      ...
      if( rc==SQLITE_OK && zName ){
        ...
        if( sz64==0 ){
          ...
          */
          if (memcmp(pOrigVfs->xAccess_signature, xAccess_signatures[xAccess_multiplexAccess_enum], sizeof(pOrigVfs->xAccess_signature)) == 0) {
          rc = multiplexAccess(pOrigVfs, pGroup->aReal[1].z,
                               SQLITE_ACCESS_EXISTS, &bExists);
          }
          if (memcmp(pOrigVfs->xAccess_signature, xAccess_signatures[xAccess_vfstraceAccess_enum], sizeof(pOrigVfs->xAccess_signature)) == 0) {
    ...
    */
    ...
    if( z==0 ){
      ...
      do{
        ...
        if (memcmp(pOrigVfs->xAccess_signature, xAccess_signatures[xAccess_multiplexAccess_enum], sizeof(pOrigVfs->xAccess_signature)) == 0) {
          rc = multiplexAccess(pOrigVfs, z, SQLITE_ACCESS_EXISTS, &bExists);
        }
        if (memcmp(pOrigVfs->xAccess_signature, xAccess_signatures[xAccess_vfstraceAccess_enum], sizeof(pOrigVfs->xAccess_signature)) == 0) {
...
static int multiplexAccess(sqlite3_vfs *a, const char *b, int c, int *d){
  if (memcmp(gMultiplex.pOrigVfs->xAccess_signature, xAccess_signatures[xAccess_multiplexAccess_enum], sizeof(gMultiplex.pOrigVfs->xAccess_signature)) == 0) {
    return multiplexAccess(gMultiplex.pOrigVfs, b, c, d);
  }
...
static int multiplexFullPathname(sqlite3_vfs *a, const char *b, int c, char *d){
  if (memcmp(gMultiplex.pOrigVfs->xFullPathname_signature, xFullPathname_signatures[xFullPathname_multiplexFullPathname_enum], sizeof(gMultiplex.pOrigVfs->xFullPathname_signature)) == 0) {
    return multiplexFullPathname(gMultiplex.pOrigVfs, b, c, d);
  }
...
static int multiplexRandomness(sqlite3_vfs *a, int b, char *c){
  if (memcmp(gMultiplex.pOrigVfs->xRandomness_signature, xRandomness_signatures[xRandomness_multiplexRandomness_enum], sizeof(gMultiplex.pOrigVfs->xRandomness_signature)) == 0) {
    return multiplexRandomness(gMultiplex.pOrigVfs, b, c);
  }
...
static int multiplexSleep(sqlite3_vfs *a, int b){
  if (memcmp(gMultiplex.pOrigVfs->xSleep_signature, xSleep_signatures[xSleep_multiplexSleep_enum], sizeof(gMultiplex.pOrigVfs->xSleep_signature)) == 0) {
    return multiplexSleep(gMultiplex.pOrigVfs, b);
  }
...
static int multiplexCurrentTime(sqlite3_vfs *a, double *b){
  if (memcmp(gMultiplex.pOrigVfs->xCurrentTime_signature, xCurrentTime_signatures[xCurrentTime_multiplexCurrentTime_enum], sizeof(gMultiplex.pOrigVfs->xCurrentTime_signature)) == 0) {
    return multiplexCurrentTime(gMultiplex.pOrigVfs, b);
  }
...
static int multiplexRead(
  ...
  if( !pGroup->bEnabled ){
    ...
    if( pSubOpen==0 ){
      ...
      if (memcmp(pSubOpen->pMethods->xRead_signature, xRead_signatures[xRead_multiplexRead_enum], sizeof(pSubOpen->pMethods->xRead_signature)) == 0) {
        rc = multiplexRead(pSubOpen, pBuf, iAmt, iOfst);
      }
    ...
    while( iAmt > 0 ){
      ...
      if( pSubOpen ){
        ...
        iAmt -= extra;
        if (memcmp(pSubOpen->pMethods->xRead_signature, xRead_signatures[xRead_multiplexRead_enum], sizeof(pSubOpen->pMethods->xRead_signature)) == 0) {
          rc = multiplexRead(pSubOpen, pBuf, iAmt, iOfst % pGroup->szChunk);
        }
        if (memcmp(pSubOpen->pMethods->xRead_signature, xRead_signatures[xRead_vfstraceRead_enum], sizeof(pSubOpen->pMethods->xRead_signature)) == 0) {
...
static int multiplexWrite(
  ...
  if( !pGroup->bEnabled ){
    ...
    if( pSubOpen==0 ){
      ...
      if (memcmp(pSubOpen->pMethods->xWrite_signature, xWrite_signatures[xWrite_multiplexWrite_enum], sizeof(pSubOpen->pMethods->xWrite_signature)) == 0) {
        rc = multiplexWrite(pSubOpen, pBuf, iAmt, iOfst);
      }
    ...
    while( rc==SQLITE_OK && iAmt>0 ){
      ...
      if( pSubOpen ){
        ...
        iAmt -= extra;
        if (memcmp(pSubOpen->pMethods->xWrite_signature, xWrite_signatures[xWrite_multiplexWrite_enum], sizeof(pSubOpen->pMethods->xWrite_signature)) == 0) {
        	rc = multiplexWrite(pSubOpen, pBuf, iAmt,
...
static int multiplexTruncate(sqlite3_file *pConn, sqlite3_int64 size){
  ...
  if( !pGroup->bEnabled ){
    ...
    if( pSubOpen==0 ){
      ...
      if (memcmp(pSubOpen->pMethods->xTruncate_signature, xTruncate_signatures[xTruncate_multiplexTruncate_enum], sizeof(pSubOpen->pMethods->xTruncate_signature)) == 0) {
        rc = multiplexTruncate(pSubOpen, size);
      }
    ...
    for(i = pGroup->nReal-1; i>iBaseGroup && rc==SQLITE_OK; i--){
      if( pGroup->bTruncate ){
        ...
        if( pSubOpen ){
          if (memcmp(pSubOpen->pMethods->xTruncate_signature, xTruncate_signatures[xTruncate_multiplexTruncate_enum], sizeof(pSubOpen->pMethods->xTruncate_signature)) == 0) {
            rc = multiplexTruncate(pSubOpen, 0);
          }
          if (memcmp(pSubOpen->pMethods->xTruncate_signature, xTruncate_signatures[xTruncate_vfstraceTruncate_enum], sizeof(pSubOpen->pMethods->xTruncate_signature)) == 0) {
    ...
    if( rc==SQLITE_OK ){
      ...
      if( pSubOpen ){
        if (memcmp(pSubOpen->pMethods->xTruncate_signature, xTruncate_signatures[xTruncate_multiplexTruncate_enum], sizeof(pSubOpen->pMethods->xTruncate_signature)) == 0) {
          rc = multiplexTruncate(pSubOpen, size % pGroup->szChunk);
        }
        if (memcmp(pSubOpen->pMethods->xTruncate_signature, xTruncate_signatures[xTruncate_vfstraceTruncate_enum], sizeof(pSubOpen->pMethods->xTruncate_signature)) == 0) {
...
static int multiplexFileSize(sqlite3_file *pConn, sqlite3_int64 *pSize){
  ...
  if( !pGroup->bEnabled ){
    ...
    if( pSubOpen==0 ){
      ...
      if (memcmp(pSubOpen->pMethods->xFileSize_signature, xFileSize_signatures[xFileSize_multiplexFileSize_enum], sizeof(pSubOpen->pMethods->xFileSize_signature)) == 0) {
        rc = multiplexFileSize(pSubOpen, pSize);
      }
      if (memcmp(pSubOpen->pMethods->xFileSize_signature, xFileSize_signatures[xFileSize_vfstraceFileSize_enum], sizeof(pSubOpen->pMethods->xFileSize_signature)) == 0) {
...
static int multiplexLock(sqlite3_file *pConn, int lock){
  ...
  if( pSubOpen ){
    if (memcmp(pSubOpen->pMethods->xLock_signature, xLock_signatures[xLock_multiplexLock_enum], sizeof(pSubOpen->pMethods->xLock_signature)) == 0) {
      return multiplexLock(pSubOpen, lock);
    }
...
static int multiplexUnlock(sqlite3_file *pConn, int lock){
  ...
  sqlite3_file *pSubOpen = multiplexSubOpen(p->pGroup, 0, &rc, NULL, 0);
  if( pSubOpen ){
    if (memcmp(pSubOpen->pMethods->xUnlock_signature, xUnlock_signatures[xUnlock_multiplexUnlock_enum], sizeof(pSubOpen->pMethods->xUnlock_signature)) == 0) {
      return multiplexUnlock(pSubOpen, lock);
    }
...
static int multiplexCheckReservedLock(sqlite3_file *pConn, int *pResOut){
  ...
  sqlite3_file *pSubOpen = multiplexSubOpen(p->pGroup, 0, &rc, NULL, 0);
  if( pSubOpen ){
    if (memcmp(pSubOpen->pMethods->xCheckReservedLock_signature, xCheckReservedLock_signatures[xCheckReservedLock_multiplexCheckReservedLock_enum], sizeof(pSubOpen->pMethods->xCheckReservedLock_signature)) == 0) {
      return multiplexCheckReservedLock(pSubOpen, pResOut);
    }
...
static int multiplexFileControl(sqlite3_file *pConn, int op, void *pArg){
  ...
  switch( op ){
    ...
    default:
      ...
      if( pSubOpen ){
        if (memcmp(pSubOpen->pMethods->xFileControl_signature, xFileControl_signatures[xFileControl_multiplexFileControl_enum], sizeof(pSubOpen->pMethods->xFileControl_signature)) == 0) {
        rc = multiplexFileControl(pSubOpen, op, pArg);
        }
...
static int multiplexSectorSize(sqlite3_file *pConn){
  ...
  if( pSubOpen && pSubOpen->pMethods->xSectorSize ){
    if (memcmp(pSubOpen->pMethods->xSectorSize_signature, xSectorSize_signatures[xSectorSize_multiplexSectorSize_enum], sizeof(pSubOpen->pMethods->xSectorSize_signature)) == 0) {
      return multiplexSectorSize(pSubOpen);
    }
...
static int multiplexDeviceCharacteristics(sqlite3_file *pConn){
  ...
  sqlite3_file *pSubOpen = multiplexSubOpen(p->pGroup, 0, &rc, NULL, 0);
  if( pSubOpen ){
    if (memcmp(pSubOpen->pMethods->xDeviceCharacteristics_signature, xDeviceCharacteristics_signatures[xDeviceCharacteristics_multiplexDeviceCharacteristics_enum], sizeof(pSubOpen->pMethods->xDeviceCharacteristics_signature)) == 0) {
      return multiplexDeviceCharacteristics(pSubOpen);
    }
...
static int multiplexShmMap(
  ...
  sqlite3_file *pSubOpen = multiplexSubOpen(p->pGroup, 0, &rc, NULL, 0);
  if( pSubOpen ){
    if (memcmp(pSubOpen->pMethods->xShmMap_signature, xShmMap_signatures[xShmMap_multiplexShmMap_enum], sizeof(pSubOpen->pMethods->xShmMap_signature)) == 0) {
      return multiplexShmMap(pSubOpen, iRegion, szRegion, bExtend, pp);
    }
  }
  return SQLITE_IOERR;
}
...
static int multiplexShmLock(
  ...
  sqlite3_file *pSubOpen = multiplexSubOpen(p->pGroup, 0, &rc, NULL, 0);
  if( pSubOpen ){
    if (memcmp(pSubOpen->pMethods->xShmLock_signature, xShmLock_signatures[xShmLock_multiplexShmLock_enum], sizeof(pSubOpen->pMethods->xShmLock_signature)) == 0) {
      return multiplexShmLock(pSubOpen, ofst, n, flags);
    }
  }
  return SQLITE_BUSY;
}
...
static void multiplexShmBarrier(sqlite3_file *pConn){
  ...
  sqlite3_file *pSubOpen = multiplexSubOpen(p->pGroup, 0, &rc, NULL, 0);
  if( pSubOpen ){
    if (memcmp(pSubOpen->pMethods->xShmBarrier_signature, xShmBarrier_signatures[xShmBarrier_multiplexShmBarrier_enum], sizeof(pSubOpen->pMethods->xShmBarrier_signature)) == 0) {
      multiplexShmBarrier(pSubOpen);
    }
  }
}
...
static int multiplexShmUnmap(sqlite3_file *pConn, int deleteFlag){
  ...
  sqlite3_file *pSubOpen = multiplexSubOpen(p->pGroup, 0, &rc, NULL, 0);
  if( pSubOpen ){
    if (memcmp(pSubOpen->pMethods->xShmUnmap_signature, xShmUnmap_signatures[xShmUnmap_multiplexShmUnmap_enum], sizeof(pSubOpen->pMethods->xShmUnmap_signature)) == 0) {
      return multiplexShmUnmap(pSubOpen, deleteFlag);
    }
  }
  return SQLITE_OK;
}
