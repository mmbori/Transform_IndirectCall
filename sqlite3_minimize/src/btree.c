...
void sqlite3BtreeCursorHint(BtCursor *pCur, int eHintType, ...){
  ...
  if( ALWAYS(eHintType==BTREE_HINT_RANGE) ){
    ...
    memcpy(w.xExprCallback_signature,
           xExprCallback_signatures[xExprCallback_sqlite3CursorRangeHintExprCheck_enum],
...
static int decodeFlags(MemPage *pPage, int flagByte){
  ...
  if( flagByte>=(PTF_ZERODATA | PTF_LEAF) ){
    ...
    if( flagByte==(PTF_LEAFDATA | PTF_INTKEY | PTF_LEAF) ){
      pPage->intKeyLeaf = 1;
      memcpy(pPage->xCellSize_signature,
             xCellSize_signatures[xCellSize_cellSizePtrTableLeaf_enum],
             sizeof(pPage->xCellSize_signature));
      memcpy(pPage->xParseCell_signature,
             xParseCell_signatures[xParseCell_btreeParseCellPtr_enum],
             sizeof(pPage->xParseCell_signature));
      pPage->intKey = 1;
      ...
      pPage->intKeyLeaf = 0;
      memcpy(pPage->xCellSize_signature,
             xCellSize_signatures[xCellSize_cellSizePtrIdxLeaf_enum],
             sizeof(pPage->xCellSize_signature));
      memcpy(pPage->xParseCell_signature,
             xParseCell_signatures[xParseCell_btreeParseCellPtrIndex_enum],
             sizeof(pPage->xParseCell_signature));
    ...
    if( flagByte==(PTF_ZERODATA) ){
      ...
      pPage->intKeyLeaf = 0;
      memcpy(pPage->xCellSize_signature,
             xCellSize_signatures[xCellSize_cellSizePtr_enum],
             sizeof(pPage->xCellSize_signature));
      memcpy(pPage->xParseCell_signature,
             xParseCell_signatures[xParseCell_btreeParseCellPtrIndex_enum],
             sizeof(pPage->xParseCell_signature));
      ...
      pPage->intKeyLeaf = 0;
      memcpy(pPage->xCellSize_signature,
             xCellSize_signatures[xCellSize_cellSizePtrNoPayload_enum],
             sizeof(pPage->xCellSize_signature));
      memcpy(pPage->xParseCell_signature,
             xParseCell_signatures[xParseCell_btreeParseCellPtrNoPayload_enum],
             sizeof(pPage->xParseCell_signature));
      pPage->intKey = 1;
      pPage->maxLocal = pBt->maxLeaf;
      pPage->minLocal = pBt->minLeaf;
...
void *sqlite3BtreeSchema(Btree *p, int nBytes, void(*xFree)(void *)){
  ...
  if( !pBt->pSchema && nBytes ){
    ...
    memcpy(pBt->xFreeSchema_signature,
           xFreeSchema_signatures[xFreeSchema_xFree_enum],
           sizeof(pBt->xFreeSchema_signature));
  }
  sqlite3BtreeLeave(p);
  return pBt->pSchema;
}
