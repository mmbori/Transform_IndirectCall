...
static void sqlite3ProcessReturningSubqueries(
  ...
  memcpy(w.xSelectCallback_signature,
         xSelectCallback_signatures[xSelectCallback_sqlite3ReturningSubqueryCorrelated_enum],
         sizeof(w.xSelectCallback_signature));
  w.u.pTab = pTab;
  sqlite3WalkExprList(&w, pEList);
  if( w.eCode ){
    memcpy(w.xExprCallback_signature,
           xExprCallback_signatures[xExprCallback_sqlite3ReturningSubqueryVarSelect_enum],
           sizeof(w.xExprCallback_signature));
    memcpy(w.xSelectCallback_signature,
