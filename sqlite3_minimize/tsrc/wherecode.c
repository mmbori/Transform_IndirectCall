...
static void codeCursorHint(
  ...
  for(i=0; i<pWC->nBase; i++){
      ...
      if( !ExprHasProperty(pExpr, EP_OuterON)
       || pExpr->w.iJoin!=pTabItem->iCursor
        ...
        memcpy(sWalker.xExprCallback_signature,
               xExprCallback_signatures[xExprCallback_codeCursorHintIsOrFunction_enum],
    ...
    ** the index. */
    if( sHint.pIdx!=0 ){
      sWalker.eCode = 0;
      memcpy(sWalker.xExprCallback_signature,
             xExprCallback_signatures[xExprCallback_codeCursorHintCheckExpr_enum],
             sizeof(sWalker.xExprCallback_signature));
      sqlite3WalkExpr(&sWalker, pTerm->pExpr);
      if( sWalker.eCode ) continue;
    }
  ...
  if( pExpr!=0 ){
    memcpy(sWalker.xExprCallback_signature,
           xExprCallback_signatures[xExprCallback_codeCursorHintFixExpr_enum],
           sizeof(sWalker.xExprCallback_signature));
