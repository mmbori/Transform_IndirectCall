...
static void gatherSelectWindows(Select *p){
  Walker w;
  memcpy(w.xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_gatherSelectWindowsCallback_enum],
         sizeof(w.xExprCallback_signature));
  memcpy(w.xSelectCallback_signature,
         xSelectCallback_signatures[xSelectCallback_gatherSelectWindowsSelectCallback_enum],
         sizeof(w.xSelectCallback_signature));
...
*/
SQLITE_NOINLINE int exprNodeIsConstantFunction(
  Walker *pWalker,
  Expr *pExpr
){
...
static int exprIsConst(Parse *pParse, Expr *p, int initFlag){
  ...
  w.pParse = pParse;
  memcpy(w.xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_exprNodeIsConstant_enum],
         sizeof(w.xExprCallback_signature));
  memcpy(w.xSelectCallback_signature,
         xSelectCallback_signatures[xSelectCallback_sqlite3SelectWalkFail_enum],
         sizeof(w.xSelectCallback_signature));
#ifdef SQLITE_DEBUG
  memcpy(w.xSelectCallback2_signature,
         xSelectCallback2_signatures[xSelectCallback2_sqlite3SelectWalkAssert2_enum],
         sizeof(w.xSelectCallback2_signature));
#endif
  sqlite3WalkExpr(&w, p);
  return w.eCode;
}
...
static int sqlite3ExprIsTableConstant(Expr *p, int iCur, int bAllowSubq){
  ...
  if( bAllowSubq ){
    memcpy(w.xSelectCallback_signature,
           xSelectCallback_signatures[xSelectCallback_exprSelectWalkTableConstant_enum],
           sizeof(w.xSelectCallback_signature));
  }else{
    memcpy(w.xSelectCallback_signature,
           xSelectCallback_signatures[xSelectCallback_sqlite3SelectWalkFail_enum],
...
int sqlite3ExprIsConstantOrGroupBy(Parse *pParse, Expr *p, ExprList *pGroupBy){
  ...
  w.eCode = 1;
  memcpy(w.xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_exprNodeIsConstantOrGroupBy_enum],
         sizeof(w.xExprCallback_signature));
  w.xSelectCallback = 0;
...
int sqlite3ExprContainsSubquery(Expr *p){
  ...
  w.eCode = 1;
  memcpy(w.xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_sqlite3ExprWalkNoop_enum],
         sizeof(w.xExprCallback_signature));
  memcpy(w.xSelectCallback_signature,
         xSelectCallback_signatures[xSelectCallback_sqlite3SelectWalkFail_enum],
         sizeof(w.xSelectCallback_signature));
...
static int sqlite3ExprCanReturnSubtype(Parse *pParse, Expr *pExpr){
  ...
  w.pParse = pParse;
  memcpy(w.xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_exprNodeCanReturnSubtype_enum],
         sizeof(w.xExprCallback_signature));
...
int sqlite3ExprImpliesNonNullRow(Expr *p, int iTab, int isRJ){
  ...
  memcpy(w.xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_impliesNotNullRow_enum],
         sizeof(w.xExprCallback_signature));
  w.xSelectCallback = 0;
  w.xSelectCallback2 = 0;
  w.eCode = 0;
...
int sqlite3ExprCoveredByIndex(
  ...
  xcov.pIdx = pIdx;
  memcpy(w.xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_exprIdxCover_enum],
         sizeof(w.xExprCallback_signature));
  w.u.pIdxCover = &xcov;
  sqlite3WalkExpr(&w, pExpr);
  return !w.eCode;
...
int sqlite3ReferencesSrcList(Parse *pParse, Expr *pExpr, SrcList *pSrcList){
  ...
  memset(&x, 0, sizeof(x));
  memcpy(w.xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_exprRefToSrcList_enum],
         sizeof(w.xExprCallback_signature));
  memcpy(w.xSelectCallback_signature,
         xSelectCallback_signatures[xSelectCallback_selectRefEnter_enum],
         sizeof(w.xSelectCallback_signature));
  memcpy(w.xSelectCallback2_signature,
         xSelectCallback2_signatures[xSelectCallback2_selectRefLeave_enum],
         sizeof(w.xSelectCallback2_signature));
  w.u.pRefSrcList = &x;
  x.db = pParse->db;
  x.pRef = pSrcList;
...
void sqlite3AggInfoPersistWalkerInit(Walker *pWalker, Parse *pParse){
  ...
  memcpy(pWalker->xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_agginfoPersistExprCb_enum],
         sizeof(pWalker->xExprCallback_signature));
  memcpy(pWalker->xSelectCallback_signature,
         xSelectCallback_signatures[xSelectCallback_sqlite3SelectWalkNoop_enum],
         sizeof(pWalker->xSelectCallback_signature));
}
...
void sqlite3ExprAnalyzeAggregates(NameContext *pNC, Expr *pExpr){
  Walker w;
  memcpy(w.xExprCallback_signature,
         xExprCallback_signatures[xExprCallback_analyzeAggregate_enum],
         sizeof(w.xExprCallback_signature));
  memcpy(w.xSelectCallback_signature,
         xSelectCallback_signatures[xSelectCallback_sqlite3WalkerDepthIncrease_enum],
         sizeof(w.xSelectCallback_signature));
  memcpy(w.xSelectCallback2_signature,
         xSelectCallback2_signatures[xSelectCallback2_sqlite3WalkerDepthDecrease_enum],
         sizeof(w.xSelectCallback2_signature));
  w.walkerDepth = 0;
  w.u.pNC = pNC;
  w.pParse = 0;
