...
void *faultsimMalloc(int n){
  void *p = 0;
  if( !faultsimStep() ){
    if (memcmp(memfault.m.xMalloc_signature, xMalloc_signatures[xMalloc_memtraceMalloc_enum], sizeof(memfault.m.xMalloc_signature)) == 0) {
      p = memtraceMalloc(n);
    }
    else
      if (memcmp(memfault.m.xMalloc_signature, xMalloc_signatures[xMalloc_sqlite3MemMalloc_enum], sizeof(memfault.m.xMalloc_signature)) == 0) {
        p = sqlite3MemMalloc(n);
      }
  }
  return p;
}
...
void *faultsimRealloc(void *pOld, int n){
  void *p = 0;
  if( !faultsimStep() ){
    if (memcmp(memfault.m.xRealloc_signature, xRealloc_signatures[xRealloc_memtraceRealloc_enum], sizeof(memfault.m.xRealloc_signature)) == 0) {
      p = memtraceRealloc(pOld, n);
    }
    else
      if (memcmp(memfault.m.xRealloc_signature, xRealloc_signatures[xRealloc_sqlite3MemRealloc_enum], sizeof(memfault.m.xRealloc_signature)) == 0) {
        p = sqlite3MemRealloc(pOld, n);
      }
  }
  return p;
}
...
**
** There are two buffers with BUFID 1 and 2.   Each static buffer
