...
void (*tvfsDlSym(sqlite3_vfs*,void*, const char *zSymbol))(void);
...
static sqlite3_io_methods tvfs_io_methods = {
  ...
  tvfsUnfetch
,
  .xClose_signature = xClose_signatures[xClose_tvfsClose_enum],
  .xRead_signature = xRead_signatures[xRead_tvfsRead_enum],
  .xWrite_signature = xWrite_signatures[xWrite_tvfsWrite_enum],
  .xTruncate_signature = xTruncate_signatures[xTruncate_tvfsTruncate_enum],
  .xSync_signature = xSync_signatures[xSync_tvfsSync_enum],
  .xFileSize_signature = xFileSize_signatures[xFileSize_tvfsFileSize_enum],
  .xLock_signature = xLock_signatures[xLock_tvfsLock_enum],
  .xUnlock_signature = xUnlock_signatures[xUnlock_tvfsUnlock_enum],
  .xCheckReservedLock_signature = xCheckReservedLock_signatures[xCheckReservedLock_tvfsCheckReservedLock_enum],
  .xFileControl_signature = xFileControl_signatures[xFileControl_tvfsFileControl_enum],
  .xSectorSize_signature = xSectorSize_signatures[xSectorSize_tvfsSectorSize_enum],
  .xDeviceCharacteristics_signature = xDeviceCharacteristics_signatures[xDeviceCharacteristics_tvfsDeviceCharacteristics_enum],
  .xShmMap_signature = xShmMap_signatures[xShmMap_tvfsShmMap_enum],
  .xShmLock_signature = xShmLock_signatures[xShmLock_tvfsShmLock_enum],
  .xShmBarrier_signature = xShmBarrier_signatures[xShmBarrier_tvfsShmBarrier_enum],
  .xShmUnmap_signature = xShmUnmap_signatures[xShmUnmap_tvfsShmUnmap_enum],
  .xFetch_signature = xFetch_signatures[xFetch_tvfsFetch_enum],
  .xUnfetch_signature = xUnfetch_signatures[xUnfetch_tvfsUnfetch_enum]
};
...
int tvfsInjectCantopenerr(Testvfs *p){
...
** Close an tvfs-file.
*/
int tvfsClose(sqlite3_file *pFile){
...
*/
int tvfsTruncate(sqlite3_file *pFile, sqlite_int64 size){
  int rc = SQLITE_OK;
  TestvfsFd *pFd = tvfsGetFd(pFile);
  Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
...
** Sync an tvfs-file.
*/
int tvfsSync(sqlite3_file *pFile, int flags){
  int rc = SQLITE_OK;
  TestvfsFd *pFd = tvfsGetFd(pFile);
  Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
...
*/
int tvfsFileSize(sqlite3_file *pFile, sqlite_int64 *pSize){
...
** Lock an tvfs-file.
*/
int tvfsLock(sqlite3_file *pFile, int eLock){
  TestvfsFd *pFd = tvfsGetFd(pFile);
  Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
...
** Unlock an tvfs-file.
*/
int tvfsUnlock(sqlite3_file *pFile, int eLock){
  TestvfsFd *pFd = tvfsGetFd(pFile);
  Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
...
*/
int tvfsCheckReservedLock(sqlite3_file *pFile, int *pResOut){
  TestvfsFd *pFd = tvfsGetFd(pFile);
  Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
...
*/
int tvfsFileControl(sqlite3_file *pFile, int op, void *pArg){
  TestvfsFd *pFd = tvfsGetFd(pFile);
  Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
...
*/
int tvfsSectorSize(sqlite3_file *pFile){
  TestvfsFd *pFd = tvfsGetFd(pFile);
  Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
  if( p->iSectorsize>=0 ){
...
*/
int tvfsDeviceCharacteristics(sqlite3_file *pFile){
  TestvfsFd *pFd = tvfsGetFd(pFile);
  Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
  if( p->iDevchar>=0 ){
    return p->iDevchar;
  ...
  */
  ...
  if( pFd->pReal->pMethods ){
    ...
    if( pVfs->iVersion>1 && ((Testvfs *)pVfs->pAppData)->isNoshm ){
      ...
      pMethods->xShmUnmap_signature = xShmUnmap_signatures[xShmUnmap_0_enum];
      pMethods->xShmLock = 0;
      pMethods->xShmLock_signature = xShmLock_signatures[xShmLock_0_enum];
      pMethods->xShmBarrier = 0;
      pMethods->xShmBarrier_signature = xShmBarrier_signatures[xShmBarrier_0_enum];
      pMethods->xShmMap = 0;
      pMethods->xShmMap_signature = xShmMap_signatures[xShmMap_0_enum];
    }
...
** returning.
*/
int tvfsDelete(sqlite3_vfs *pVfs, const char *zPath, int dirSync){
  int rc = SQLITE_OK;
  Testvfs *p = (Testvfs *)pVfs->pAppData;
...
*/
void tvfsDlError(sqlite3_vfs *pVfs, int nByte, char *zErrMsg){
...
*/
void (*tvfsDlSym(sqlite3_vfs *pVfs, void *p, const char *zSym))(void){
...
*/
void tvfsDlClose(sqlite3_vfs *pVfs, void *pHandle){
...
** actually slept.
*/
int tvfsSleep(sqlite3_vfs *pVfs, int nMicro){
  Testvfs *p = (Testvfs *)pVfs->pAppData;
...
*/
int tvfsCurrentTime(sqlite3_vfs *pVfs, double *pTimeOut){
...
void tvfsShmBarrier(sqlite3_file *pFile){
  TestvfsFd *pFd = tvfsGetFd(pFile);
  Testvfs *p = (Testvfs *)(pFd->pVfs->pAppData);
...
int tvfsUnfetch(sqlite3_file *pFile, sqlite3_int64 iOfst, void *p){
  TestvfsFd *pFd = tvfsGetFd(pFile);
...
static int SQLITE_TCLAPI testvfs_cmd(
  ...
  static sqlite3_vfs tvfs_vfs = {
    ...
    0,                            /* xNextSystemCall */
  ,
  .xOpen_signature = xOpen_signatures[xOpen_tvfsOpen_enum],
  .xDelete_signature = xDelete_signatures[xDelete_tvfsDelete_enum],
  .xAccess_signature = xAccess_signatures[xAccess_tvfsAccess_enum],
  .xFullPathname_signature = xFullPathname_signatures[xFullPathname_tvfsFullPathname_enum],
  .xDlError_signature = xDlError_signatures[xDlError_tvfsDlError_enum],
  .xDlSym_signature = xDlSym_signatures[xDlSym_tvfsDlSym_enum],
  .xDlClose_signature = xDlClose_signatures[xDlClose_tvfsDlClose_enum],
  .xSetSystemCall_signature = xSetSystemCall_signatures[xSetSystemCall_tvfsSleep_enum],
  .xGetSystemCall_signature = xGetSystemCall_signatures[xGetSystemCall_tvfsCurrentTime_enum]
};
