...
static void intarrayFree(void *pX){
  ...
  if( p->xFree ){
    if (memcmp(p->xFree_signature, xFree_signatures[xFree_memtraceFree_enum], sizeof(p->xFree_signature)) == 0) {
      memtraceFree(p->a);
    }
    else
      if (memcmp(p->xFree_signature, xFree_signatures[xFree_sqlite3MemFree_enum], sizeof(p->xFree_signature)) == 0) {
        sqlite3MemFree(p->a);
      }
  }
  sqlite3_free(p);
...
static sqlite3_module intarrayModule = {
  ...
  0                            /* xIntegrity */
,
  .xCreate_signature = xCreate_signatures[xCreate_intarrayCreate_enum],
  .xConnect_signature = xConnect_signatures[xConnect_intarrayCreate_enum],
  .xBestIndex_signature = xBestIndex_signatures[xBestIndex_intarrayBestIndex_enum],
  .xDisconnect_signature = xDisconnect_signatures[xDisconnect_intarrayDestroy_enum],
  .xDestroy_signature = xDestroy_signatures[xDestroy_intarrayDestroy_enum],
  .xOpen_signature = xOpen_signatures[xOpen_intarrayOpen_enum],
  .xClose_signature = xClose_signatures[xClose_intarrayClose_enum],
  .xFilter_signature = xFilter_signatures[xFilter_intarrayFilter_enum],
  .xNext_signature = xNext_signatures[xNext_intarrayNext_enum],
  .xEof_signature = xEof_signatures[xEof_intarrayEof_enum],
  .xColumn_signature = xColumn_signatures[xColumn_intarrayColumn_enum],
  .xRowid_signature = xRowid_signatures[xRowid_intarrayRowid_enum]
};
