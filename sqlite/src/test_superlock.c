...
static int superlockWalLock(
  ...
  ** same reason.
  */
  if (memcmp(fd->pMethods->xShmMap_signature, xShmMap_signatures[xShmMap_0_enum], sizeof(fd->pMethods->xShmMap_signature)) == 0) {
    rc = 0;
  }
  else
    if (memcmp(fd->pMethods->xShmMap_signature, xShmMap_signatures[xShmMap_apndShmMap_enum], sizeof(fd->pMethods->xShmMap_signature)) == 0) {
      rc = apndShmMap(fd, 0, 32 * 1024, 1, &p);
    }
  else
    if (memcmp(fd->pMethods->xShmMap_signature, xShmMap_signatures[xShmMap_recoverVfsShmMap_enum], sizeof(fd->pMethods->xShmMap_signature)) == 0) {
      rc = recoverVfsShmMap(fd, 0, 32 * 1024, 1, &p);
    }
  else
    if (memcmp(fd->pMethods->xShmMap_signature, xShmMap_signatures[xShmMap_unixShmMap_enum], sizeof(fd->pMethods->xShmMap_signature)) == 0) {
      rc = unixShmMap(fd, 0, 32 * 1024, 1, &p);
    }
  if( rc!=SQLITE_OK ) return rc;
  memset((void *)p, 0, 32);
...
void sqlite3demo_superunlock(void *pLock){
  ...
  if( p->bWal ){
    ...
    if( rc==SQLITE_OK ){
      if (memcmp(fd->pMethods->xShmLock_signature, xShmLock_signatures[xShmLock_0_enum], sizeof(fd->pMethods->xShmLock_signature)) == 0) {
        0;
      }
      else
        if (memcmp(fd->pMethods->xShmLock_signature, xShmLock_signatures[xShmLock_apndShmLock_enum], sizeof(fd->pMethods->xShmLock_signature)) == 0) {
          apndShmLock(fd, 2, 1, flags);
        }
      else
        if (memcmp(fd->pMethods->xShmLock_signature, xShmLock_signatures[xShmLock_recoverVfsShmLock_enum], sizeof(fd->pMethods->xShmLock_signature)) == 0) {
          recoverVfsShmLock(fd, 2, 1, flags);
        }
      else
        if (memcmp(fd->pMethods->xShmLock_signature, xShmLock_signatures[xShmLock_unixShmLock_enum], sizeof(fd->pMethods->xShmLock_signature)) == 0) {
          unixShmLock(fd, 2, 1, flags);
        }
      if (memcmp(fd->pMethods->xShmLock_signature, xShmLock_signatures[xShmLock_0_enum], sizeof(fd->pMethods->xShmLock_signature)) == 0) {
        0;
      }
        ...
        if (memcmp(fd->pMethods->xShmLock_signature, xShmLock_signatures[xShmLock_apndShmLock_enum], sizeof(fd->pMethods->xShmLock_signature)) == 0) {
          apndShmLock(fd, 3, SQLITE_SHM_NLOCK - 3, flags);
        }
      else
        if (memcmp(fd->pMethods->xShmLock_signature, xShmLock_signatures[xShmLock_recoverVfsShmLock_enum], sizeof(fd->pMethods->xShmLock_signature)) == 0) {
          recoverVfsShmLock(fd, 3, SQLITE_SHM_NLOCK - 3, flags);
        }
      else
        if (memcmp(fd->pMethods->xShmLock_signature, xShmLock_signatures[xShmLock_unixShmLock_enum], sizeof(fd->pMethods->xShmLock_signature)) == 0) {
          unixShmLock(fd, 3, SQLITE_SHM_NLOCK - 3, flags);
        }
    }
  }
  sqlite3_close(p->db);
