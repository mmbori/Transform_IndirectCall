...
def check_fpname_declarations():
    ...
    for decl_file in decl_files:
        ...
        try:
            ...
            # "선언을 찾을 수 없음" 패턴 검색 (콜론 유무 상관없이)
            # 여러 패턴을 시도
            has_missing = False
            
            # 패턴 1: "// 선언을 찾을 수 없음" 또는 "// 선언을 찾을 수 없음:"
            if '선언을 찾을 수 없음' in content:
                has_missing = True
            
            # 패턴 2: 파일 내용이 거의 비어있거나 선언이 없는 경우
            # 의미있는 선언이 있는지 확인
            lines = [line.strip() for line in content.split('\n') 
                    if line.strip() and not line.strip().startswith('//') 
                    and not line.strip().startswith('#')]
            
            # 실제 함수 선언(세미콜론으로 끝나는 라인)이 있는지 확인
            declaration_lines = [line for line in lines if line.endswith(';')]
            
            if has_missing:
                # 누락된 선언이 있는 경우
                incomplete_fp_names.append(fp_name)
                print(f"[INCOMPLETE] {fp_name}: 일부 선언 누락")
            elif len(declaration_lines) == 0:
                # 선언이 하나도 없는 경우
                incomplete_fp_names.append(fp_name)
                print(f"[INCOMPLETE] {fp_name}: 선언 없음 (빈 파일)")
            else:
                # 모든 선언이 완전한 경우
                complete_fp_names.append(fp_name)
                print(f"[COMPLETE] {fp_name}: {len(declaration_lines)}개 선언 완료")
    ...
    if incomplete_fp_names:
        print(f"\n불완전한 함수 포인터 목록:")
        for fp_name in sorted(incomplete_fp_names)[:20]:  # 처음 20개만 표시
            print(f"  - {fp_name}")
        if len(incomplete_fp_names) > 20:
            print(f"  ... 외 {len(incomplete_fp_names) - 20}개")
...
def show_detailed_analysis():
    ...
    for decl_file in decl_files:
        ...
        try:
            ...
            # 누락 확인
            has_missing = '선언을 찾을 수 없음' in content
            ...
            if has_missing:
                missing_count += 1
            elif len(function_lines) == 0:
                empty_count += 1
                print(f"  {fp_name}: 선언 없음 (빈 파일)")
    ...
    print(f"\n전체 통계:")
    print(f"  함수 포인터: {total_fps}개")
    print(f"  선언이 있는 함수: {total_functions}개")
    print(f"  누락 표시가 있는 FP: {missing_count}개")
    print(f"  빈 파일: {empty_count}개")
    complete_count = total_fps - missing_count - empty_count
    if total_fps > 0:
        print(f"  완성된 FP: {complete_count}개 ({(complete_count / total_fps * 100):.1f}%)")
...
def verify_fpname_files(complete_fp_names):
    ...
    for fp_name in complete_fp_names:
        ...
        if os.path.exists(fpname_file):
            try:
                with open(fpname_file, 'r', encoding='utf-8', errors='ignore') as f:
    ...
    if missing_fpname_files:
        print(f"\n[WARNING] {len(missing_fpname_files)}개 함수 포인터의 fpName 파일이 없습니다:")
        for fp_name in missing_fpname_files[:10]:  # 처음 10개만 표시
            print(f"  - {fp_name}")
        if len(missing_fpname_files) > 10:
            print(f"  ... 외 {len(missing_fpname_files) - 10}개")
