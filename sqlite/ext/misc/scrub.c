...
static u8 *scrubBackupRead(ScrubState *p, int pgno, u8 *pBuf){
  ...
  if (memcmp(p->pSrc->pMethods->xRead_signature, xRead_signatures[xRead_apndRead_enum], sizeof(p->pSrc->pMethods->xRead_signature)) == 0) {
    rc = apndRead(p->pSrc, pOut, p->szPage, iOff);
  }
  else
    if (memcmp(p->pSrc->pMethods->xRead_signature, xRead_signatures[xRead_memdbRead_enum], sizeof(p->pSrc->pMethods->xRead_signature)) == 0) {
      rc = memdbRead(p->pSrc, pOut, p->szPage, iOff);
    }
  else
    if (memcmp(p->pSrc->pMethods->xRead_signature, xRead_signatures[xRead_memjrnlRead_enum], sizeof(p->pSrc->pMethods->xRead_signature)) == 0) {
      rc = memjrnlRead(p->pSrc, pOut, p->szPage, iOff);
    }
  else
    if (memcmp(p->pSrc->pMethods->xRead_signature, xRead_signatures[xRead_recoverVfsRead_enum], sizeof(p->pSrc->pMethods->xRead_signature)) == 0) {
      rc = recoverVfsRead(p->pSrc, pOut, p->szPage, iOff);
    }
  else
    if (memcmp(p->pSrc->pMethods->xRead_signature, xRead_signatures[xRead_vfstraceRead_enum], sizeof(p->pSrc->pMethods->xRead_signature)) == 0) {
      rc = vfstraceRead(p->pSrc, pOut, p->szPage, iOff);
    }
  else
    if (memcmp(p->pSrc->pMethods->xRead_signature, xRead_signatures[xRead_unixRead_enum], sizeof(p->pSrc->pMethods->xRead_signature)) == 0) {
      rc = unixRead(p->pSrc, pOut, p->szPage, iOff);
    }
  if( rc!=SQLITE_OK ){
...
static void scrubBackupWrite(ScrubState *p, int pgno, const u8 *pData){
  ...
  if( p->rcErr ) return;
  iOff = (pgno-1)*(sqlite3_int64)p->szPage;
  if (memcmp(p->pDest->pMethods->xWrite_signature, xWrite_signatures[xWrite_apndWrite_enum], sizeof(p->pDest->pMethods->xWrite_signature)) == 0) {
    rc = apndWrite(p->pDest, pData, p->szPage, iOff);
  }
  else
    if (memcmp(p->pDest->pMethods->xWrite_signature, xWrite_signatures[xWrite_kvstorageWrite_enum], sizeof(p->pDest->pMethods->xWrite_signature)) == 0) {
      rc = kvstorageWrite(p->pDest, pData, p->szPage, iOff);
    }
  else
    if (memcmp(p->pDest->pMethods->xWrite_signature, xWrite_signatures[xWrite_memdbWrite_enum], sizeof(p->pDest->pMethods->xWrite_signature)) == 0) {
      rc = memdbWrite(p->pDest, pData, p->szPage, iOff);
    }
  else
    if (memcmp(p->pDest->pMethods->xWrite_signature, xWrite_signatures[xWrite_memjrnlWrite_enum], sizeof(p->pDest->pMethods->xWrite_signature)) == 0) {
      rc = memjrnlWrite(p->pDest, pData, p->szPage, iOff);
    }
  else
    if (memcmp(p->pDest->pMethods->xWrite_signature, xWrite_signatures[xWrite_recoverVfsWrite_enum], sizeof(p->pDest->pMethods->xWrite_signature)) == 0) {
      rc = recoverVfsWrite(p->pDest, pData, p->szPage, iOff);
    }
  else
    if (memcmp(p->pDest->pMethods->xWrite_signature, xWrite_signatures[xWrite_vfstraceWrite_enum], sizeof(p->pDest->pMethods->xWrite_signature)) == 0) {
      rc = vfstraceWrite(p->pDest, pData, p->szPage, iOff);
    }
  else
    if (memcmp(p->pDest->pMethods->xWrite_signature, xWrite_signatures[xWrite_unixWrite_enum], sizeof(p->pDest->pMethods->xWrite_signature)) == 0) {
      rc = unixWrite(p->pDest, pData, p->szPage, iOff);
    }
  if( rc!=SQLITE_OK ){
