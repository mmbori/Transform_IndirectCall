...
void setGetterMethod(Pager *pPager){
  if( pPager->errCode ){
    memcpy(pPager->xGet_signature, xGet_signatures[xGet_getPageError_enum],
           sizeof(pPager->xGet_signature));
#if SQLITE_MAX_MMAP_SIZE>0
  }else if( USEFETCH(pPager) ){
    memcpy(pPager->xGet_signature, xGet_signatures[xGet_getPageMMap_enum],
           sizeof(pPager->xGet_signature));
#endif /* SQLITE_MAX_MMAP_SIZE>0 */
  }else{
    memcpy(pPager->xGet_signature, xGet_signatures[xGet_getPageNormal_enum],
           sizeof(pPager->xGet_signature));
  }
}
